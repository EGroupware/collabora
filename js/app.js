import { filemanagerAPP } from '../../filemanager/js/app.js';
import { w as egw_get_file_editor_prefered_mimes, d as et2_createWidget, b as et2_dialog, E as EgwApp, x as et2_IInput } from '../../chunks/etemplate2-0eb045cf.js';
...
class collaboraFilemanagerAPP extends filemanagerAPP {
  ...
  open(action, selected) {
    ...
    let dblclick_action = egw.preference('document_doubleclick_action', 'filemanager'); // Check to see if it's something we can handle

    if (is_collabora && this.isEditable(action, selected) && (!dblclick_action || dblclick_action == 'collabora')) {
  ...
  _dialog_create_new(_type, _openasnew, _path) {
    ...
    let self = this;
    let ext_default = 'odt';
    let title = _openasnew ? this.egw.lang('Open as new') : this.egw.lang('Create new %1', type == 'more' ? this.egw.lang('file') : this.egw.lang(type)); // Make first char uppercase, as some languages (German) put the type first

    title = title.charAt(0).toUpperCase() + title.slice(1); //list of extensions that we don't want to offer for create new file
    ...
    switch (type) {
      ...
      case 'drawing':
        extensions = {
          odg: '(.odg) OpenDocument Drawing Document Format',
          otg: '(.otg) OpenDocument Drawing Template Format',
          fodg: '(.fodg) OpenDocument Flat XML Drawing Format'
        };
        ext_default = 'odg';
        break;
    ...
    et2_createWidget("dialog", {
      ...
      buttons: [{
        id: 'create',
        text: egw.lang('Create'),
        image: 'new',
        default: true
      }, {
        id: 'cancel',
        text: egw.lang('Cancel'),
        image: 'close'
      }],
      minWidth: 300,
      minHeight: 200,
...
class collaboraAPP extends EgwApp {
  ...
  editor_iframe = null; // Flag for if we've customized & bound the editor

  loaded = false; // Mime types supported for export/save as the openned file
   ...
   */
  constructor() {
    super("collabora"); // Filemanager has some handy utilites, but we need to be careful what
  ...
  getWindowTitle() {
    return egw.config('site_title', 'phpgwapi') + '[' + this.et2.getArrayMgr('content').getEntry('path', true) + ']';
  }
  /**
  ...
  init_editor(values) {
    ...
    frameholder.append(frame); // Listen for messages
    ...
    switch (message.MessageId) {
      case "App_LoadingStatus":
        if (message.Values.Status === 'Document_Loaded' && !this.loaded) {
          // Tell the iframe that we are ready now
          app.collabora.WOPIPostMessage('Host_PostmessageReady', {}); // Get supported export formats

          app.collabora.WOPIPostMessage('Get_Export_Formats'); // enable FollowUser option by default
        ...
        if (message.Values.Status === 'Frame_Ready') {
          if (window.opener && window.opener.app && window.opener.app.filemanager) {
            let nm = window.opener.app.filemanager.et2.getWidgetById('nm'); // try to referesh opener nm list, it helps to keep the list up to date
            // in save as changes.
  ...
  _customize_editor() {
    let baseUrl = egw.webserverUrl[0] == '/' ? window.location.protocol + '//' + window.location.hostname + egw.webserverUrl : egw.webserverUrl;
  ...
  on_close() {
    ...
    this.et2.iterateOver(function (w) {
      w.resetDirty();
    }, this, et2_IInput);
    window.close();
  }
  ...
  on_save_as() {
    ...
    let ext = parts.pop();
    let filename = parts.join('.'); // select current mime-type
    ...
    let self = this; // bind change handler for setting the selected path and calling save

    window.setTimeout(function () {
      jQuery(vfs_select.getDOMNode()).on('change', function () {
        let file_path = vfs_select.get_value();
        let selectedMime = self.export_formats[vfs_select.dialog.options.value.content.mime]; // only add extension, if not already there
    ...
    }, 1); // start the file selector dialog
  ...
  insert_image() {
    let image_selected = function (node, widget) {
      if (widget.get_value()) {
        ...
        this.egw.json('EGroupware\\Api\\Sharing::ajax_create', ['collabora', widget.get_value(), false, false, {
          share_expires: date('Y-m-d', expires)
        }], function (value) {
          // Tell Collabora about it - add '/' to the end to avoid redirect by WebDAV server
          // (WebDAV/Server.php line 247
          this.WOPIPostMessage('Action_InsertGraphic', {
            url: value.share_link + '/'
          });
